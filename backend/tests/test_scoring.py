...
def test_taint_preserves_unequal_branches(scored_db):
    conn = get_db_connection()
    conn.execute("""INSERT INTO transactions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                 ("tx_branch", 1700002000, "UNKNOWN", "UNKNOWN", 8333, 8333, .001, "p2wpkh", "UNKNOWN", "UNKNOWN",
                  json.dumps([{"address": "illicit_seed_1", "amount": 10.0}]),
                  json.dumps([{"address": "branch_major", "amount": 9.0}, {"address": "branch_minor", "amount": 1.0}]), 10.0, 9.999))
    conn.commit(); conn.close()
    scores = RiskPropagationEngine(decay_factor=.8, max_hops=1).propagate_taint()
    assert scores["branch_major"] > scores["branch_minor"] > 0
    assert scores["branch_major"] <= 1.0
