...
class OfflineGeoIPResolver:
    """Offline IPv4 country/ASN resolver with an honest UNKNOWN fallback."""
    def __init__(self, geoip_csv_path: str | None = None):
        self.example_table = {
            "198.51.100": ("RU", "AS12389 - Rostelecom"), "203.0.113": ("IR", "AS58224 - TIC"),
            "192.0.2": ("NL", "AS49981 - WorldStream"), "104.28": ("US", "AS13335 - Cloudflare"),
            "185.220": ("DE", "AS205100 - Tor Exit Node"), "194.26": ("SC", "AS51852 - Private Layer"),
        }
        self._range_starts, self._range_ends = [], []
        self._range_country, self._range_asn = [], []
        path = Path(geoip_csv_path) if geoip_csv_path else Path(__file__).resolve().parents[2] / "data" / "geoip_database.csv"
        if path.exists():
            self._load_range_database(path)

    def _load_range_database(self, path: Path) -> None:
        rows = []
        try:
            with path.open("r", encoding="utf-8", newline="") as stream:
                for row in csv.DictReader(stream):
                    start, end = int(row["ip_from"]), int(row["ip_to"])
                    if start <= end:
                        rows.append((start, end, (row.get("country_code") or "UNKNOWN").strip() or "UNKNOWN", (row.get("asn_name") or "UNKNOWN").strip() or "UNKNOWN"))
        except (OSError, ValueError, KeyError):
            return
        rows.sort(key=lambda item: item[0])
        self._range_starts = [item[0] for item in rows]
        self._range_ends = [item[1] for item in rows]
        self._range_country = [item[2] for item in rows]
        self._range_asn = [item[3] for item in rows]

    def resolve(self, ip: str) -> Tuple[str, str]:
        ip_int = _ip_to_int(ip or "")
        if ip_int is not None and self._range_starts:
            index = bisect.bisect_right(self._range_starts, ip_int) - 1
            if index >= 0 and self._range_starts[index] <= ip_int <= self._range_ends[index]:
                return self._range_country[index], self._range_asn[index]
        parts = (ip or "").strip().split(".")
        if len(parts) == 4:
            return self.example_table.get(".".join(parts[:3]), self.example_table.get(".".join(parts[:2]), ("UNKNOWN", "UNKNOWN")))
...
class BulkDataParser:
    ...
    def _validate_records(self, records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Quarantine structurally invalid observations instead of inventing values."""
        valid = []
        seen = set()
        for record in records:
            txid = record.get("txid")
            amounts = [float(item.get("amount", 0.0)) for item in record.get("inputs", []) + record.get("outputs", [])]
            ip_valid = all(value == "UNKNOWN" or _ip_to_int(value) is not None for value in (record.get("src_ip"), record.get("dst_ip")))
            if not txid or txid in seen or not ip_valid or min(amounts or [0.0]) < 0 or int(record.get("timestamp", 0)) < 0:
                self.rejected_records.append({"txid": txid, "reason": "duplicate/missing identifier, invalid IP, negative amount, or negative timestamp"})
                continue
            seen.add(txid)
            valid.append(record)
        return valid
    ...
    def parse_file(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        return self._validate_records(parsers[suffix](file_path))
