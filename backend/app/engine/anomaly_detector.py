...
class TransactionAnomalyDetector:
    ...
    def run_anomaly_detection(self) -> Dict[str, Dict[str, Any]]:
        ...
        if len(rows) < 5:
            conn.close()
            return {r["txid"]: {"anomaly_score": None, "status": "insufficient_data", "primary_deviance": None, "z_score": None,
                               "model_metadata": {"algorithm": "IsolationForest", "minimum_sample_count": 5, "sample_count": len(rows), "random_seed": 42}}
                    for r in rows}
        ...
        for idx, row in enumerate(rows):
            ...
            results[row["txid"]] = {
                ...
                "status": "scored",
                "model_metadata": {"algorithm": "IsolationForest", "contamination": self.model.contamination,
                                   "sample_count": len(rows), "random_seed": 42, "feature_names": self.feature_names}
            }
