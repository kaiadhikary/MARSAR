...
def generate_investigative_alerts() -> List[Dict[str, Any]]:
    ...
    for r in tx_rows:
        ...
        # Demixing
        is_peeling, peel_conf, peel_meta = detector.detect_peeling_chain(inputs, outputs)
        ...
        anom_res = anomaly_map.get(txid, {"anomaly_score": 0.05, "primary_deviance": "none", "z_score": 0.0})
        anom_score = anom_res["anomaly_score"]
        taint_score = taint_map.get(txid, 0.0)

        # Geo/ASN Risk
        geo_risk = 0.85 if country in ("RU", "IR", "KP") else 0.0
        demix_conf = peel_conf if is_peeling else (mix_conf if is_mixer else 0.0)
        ...
        confidence = max(demix_conf, ml_prob, anom_score, taint_score)

        if composite_risk >= 0.35 or is_peeling or is_mixer or ml_prob >= 0.65:
            if is_peeling:
            ...
            explanation_payload = {
                ...
                "risk_attribution": {
                    ...
                    "peeling_chain_evidence": peel_meta if is_peeling else None,
