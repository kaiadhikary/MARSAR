...
def local_login(req: LoginRequest):
    ...
    if not settings.FORENSIC_SECRET_KEY:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Authentication is not configured. Set MARSAR_FORENSIC_SECRET_KEY locally.")
    if not __import__("hmac").compare_digest(req.passkey, settings.FORENSIC_SECRET_KEY):
        raise HTTPException(
